// Content script for the OpenBrowser MCP extension.
...
(function () {
  if (window.__openbrowserLoaded) return;
  window.__openbrowserLoaded = true;
  ...
  function generateAccessibilityTree(options = {}) {
    ...
    const maxChars = options.max_chars || 20000;
  ...
  // Bounding box in viewport coordinates, so a caller can crop a zoom region
  // out of the last screenshot without a second round trip.
  function getRefBox(refId) {
    const el = resolveRef(refId);
    if (!el) return null;
    const rect = el.getBoundingClientRect();
    return {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === "getRefBox") {
      const result = getRefBox(msg.ref);
      sendResponse({ result });
      return true;
    }
